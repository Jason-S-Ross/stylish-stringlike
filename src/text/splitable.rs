...
impl<'a> Splitable<'a, &'a str> for String {
    type Delim = Cow<'a, str>;
    type Segment = Cow<'a, str>;
    fn split_style(
        &'a self,
        pattern: &'a str,
    ) -> Box<dyn Iterator<Item = Split<Self::Delim, Self::Segment>> + 'a> {
        Box::new(
            self.match_indices(pattern)
                ...
                .scan(0, move |last_end, item| {
                    if let Some((start, pat)) = item {
                        let end = start + pat.len();
                        let delim = Some(Cow::Borrowed(&self[start..end]));
                        let res = if start == 0 {
                            // String starts with delimiter
                            Some(Split {
                                segment: None,
                            ...
                            Some(Split {
                                segment: Some(Cow::Borrowed(&self[*last_end..start])),
                                delim,
                            })
                        };
                        *last_end = end;
                        ...
                        if *last_end == self.len() {
                            ...
                            // After consuming the last match, we still have some string yet
                            Some(Split {
                                segment: Some(Cow::Borrowed(&self[*last_end..])),
                                delim: None,
                            })
                        }
                    }
